...
export const handleBuyToken = async (account, ref) => {
  ...
  try {
    ...
    let es;
    try {
      es = await contract.estimateGas.buyToken(ref, {
        from: account,
        value: "1000000000000",
      });
    } catch (err) {
      console.log("errrr is---->", err);
      toast.error("Error while buying.." + err?.toString());
    ...
    let data = await contract.buyToken(ref, {
      from: account,
    ...
    } else toast.error("Error while buying..");
    return data;
  } catch (err) {
...
export const withdrawLevelIncome = async (account) => {
  ...
  try {
    ...
    try {
      ...
      toast.error("Error while buying.." + err);
      console.log("error", err.code);
      return false;
    }
    let priceLimit = new BigNumber(es.toString()).plus(
    ...
    let data = await contract.withdrawLevelIncome({
      from: account,
      gasLimit: Math.ceil(parseFloat(priceLimit.toString())),
    });
    data = await data.wait();
...
export const withdrawReferralIncome = async (account) => {
  ...
  try {
    ...
    let data = await contract.withdrawReferralIncome({
      from: account,
      gasLimit: Math.ceil(parseFloat(priceLimit.toString())),
    });
    data = await data.wait();
...
export const checkNetwork = async () => {
  if (window.ethereum) {
    ...
    console.log(
      "current chain id is",
      currentChainId,
      currentChainId == targetNetworkId
    );
